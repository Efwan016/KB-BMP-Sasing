import type { SoalAplikasiKonteks, Opsi } from './types';

interface Props {
  soal: SoalAplikasiKonteks[];
  showKunci: boolean;
  jawabanUser: Record<number, string>;
  onJawab: (no: number, label: string) => void;
}

const hurufToEmoji: Record<string, string> = {
  a: '🅰',
  b: '🅱',
  c: '🅲',
  d: '🅳',
...
export default function SoalAplikasiKonteks({ soal, showKunci, jawabanUser, onJawab }: Props) {
  return (
    <div className="soal-aplikasi-konteks">
      {soal.map((s) => (
            ...
            {s.opsi.map((o: Opsi) => (
              <label key={o.label} className="flex cursor-pointer items-start gap-2.5 border border-[var(--line)] bg-white/30 px-3 py-2.5 text-[13px] leading-6 text-[var(--ink)] transition hover:border-[var(--coral)] hover:bg-white/60 has-[:checked]:border-[var(--coral)] has-[:checked]:bg-[color-mix(in_srgb,var(--coral)_8%,transparent)] has-[:disabled]:cursor-not-allowed sm:px-3.5">
                <input
                  ...
                  disabled={showKunci}
                />
                <span className="min-w-5 shrink-0 font-bold text-[var(--coral)]">{hurufToEmoji[o.label] ?? o.label}.</span>
